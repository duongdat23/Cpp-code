...
public:
   ...
   // Shape operator+(Shape& s);
    ...
    // xếp chồng toán tử <<: toán tử hoawjfc hàm friend trong lớp shape thì có thể truy cập vào các thành viên trong peivate của lớp
    friend std::ostream& operator<<(std::ostream& os, const Shape& s);
    friend Shape operator+(Shape& lhs, Shape& rhs);
};
Shape operator+(Shape& lhs, Shape& rhs)
{
    // nếu ko bỏ toán tử trong lớp Shape thì sẽ bị lỗi vfi compiler ko biết chọn phần nào
    Shape res;
    std::cout << "Operator in Global : \n";
    res.set(lhs.pos_x + rhs.pos_x, lhs.pos_y + rhs.pos_y, lhs.front_color, rhs.back_color);
    res.set_name(lhs.name + "+" + rhs.name);
    return res; 
}
std::ostream& operator<<(std::ostream& os, const Shape& s)
{
    // chú ý do operator << là hàm friend lớp Shape
    // trong đó hàm operator << ta có thể truy cập trực tiếp vào các thành viên của Shape
    os << s.name << ";position: (" << s.pos_x << "," << s.pos_y << ")"
        << ";color:front=" << s.front_color << ";background=" << s.back_color;
    if (s.connected_shape)
        os << ";connect to " << s.connected_shape->get_name() << std::endl;
    else
        os << ";connect to nothing" << std::endl;
    return os;
}
// comment để bỏ
...
int main()
{
    ...
    // a.print();
    std::cout << a; // in kiểu toán tử xếp chồng
